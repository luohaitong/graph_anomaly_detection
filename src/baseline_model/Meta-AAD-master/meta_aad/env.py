...
class EnvBase(gym.Env):
    def __init__(self, datapath="data/toy.csv", budget=100):
        ...
        #self.X_train,_,self.labels,_ = train_test_split(X_train,labels,test_size=0.9,random_state=0,stratify=labels)
        ...
        for i in range(int(self.X_train.shape[0]/batch_size)+1):
            if i!= int(self.X_train.shape[0]/batch_size):
                X_train_temp = self.X_train[i*batch_size:(i+1)*batch_size,:]
            else:
                X_train_temp = self.X_train[i * batch_size:, :]
            if i==0:
                self.distances = euclidean_distances(X_train_temp, self.X_train)
                ...
                print("1")
                distances_temp = euclidean_distances(X_train_temp, X_train)
                print("2")
                distances_temp = (distances_temp - np.mean(distances_temp, axis=1)[:,None]) / np.std(distances_temp, axis=1)[:,None]
                print("3")
                print(self.distances.shape)
                nearest_neighbors_temp = np.argpartition(distances_temp, 10)[:,:10]
                print("4")
                self.distances = np.concatenate((self.distances, distances_temp), axis=0)
                self.nearest_neighbors = np.concatenate((self.nearest_neighbors, nearest_neighbors_temp), axis=0)
        print(self.nearest_neighbors.shape)

        '''
        print("x_train.shape:",self.X_train.shape)
        print("X_train:",self.X_train.nbytes / (1024 * 1024 * 1024))
        self.distances = euclidean_distances(self.X_train, self.X_train)
        print("distances:",self.distances.nbytes / (1024 * 1024 * 1024))
        #self.distances = self.distances.astype(np.float16)
        print("distances:",self.distances.nbytes / (1024 * 1024 * 1024))
        self.distances = (self.distances - np.mean(self.distances, axis=1)[:,None]) / np.std(self.distances, axis=1)[:,None]
        self.nearest_neighbors = np.argpartition(self.distances, 10)[:,:10]
        '''
        #tem1 = (self.distances - np.mean(self.distances, axis=1)[:, None])
        #tem2 = np.std(self.distances, axis=1)[:,None]
        #self.distances = tem1/tem2
        '''
        num = 0
        for i in range(len(self.X_train)):
            for j in range(len(self.X_train[i])):
                if self.X_train[i][j] == float(0):
                    num+=1
        print(num)
        print(num/(self.X_train.shape[0]*self.X_train.shape[1]))
        '''
        '''
        self.distances = (self.distances - np.mean(self.distances, axis=1)[:, None]) / np.std(self.distances, axis=1)[:,None]
        print(self.distances.nbytes/(1024*1024*1024))
        '''
        '''
        batch_size = 5000
        for i in range(int(self.distances.shape[0]/batch_size)+1):
            if i!= int(self.distances.shape[0]/batch_size):
                distances_temp = self.distances[i*batch_size:(i+1)*batch_size,:]
            else:
                distances_temp = self.distances[i * batch_size:, :]
            if i==0:
                self.nearest_neighbors = np.argpartition(distances_temp, 10)[:,:10]
            else:
                nearest_neighbors_temp = np.argpartition(distances_temp, 10)[:,:10]
                self.nearest_neighbors = np.concatenate((self.nearest_neighbors,nearest_neighbors_temp),axis=0)
            '''
